...
약결합 : 여러 처리기와 저장 장치(메모리)가 각각 존재해 원격프로시저호출(Remote Procedure Call, RPC)로 통신을 하여 서로 다른 시스템의 파일을 참조한다. 처리기 각자는 자신의 메모리에 대한 접근시간은 매우 빠르나 다른 처리기의 메모리는 통신선을 통해 접근해야 하므로 늦다. 일반적으로 병렬처리 시스템으로 분류한다.

밀결합 : 여러 처리기 간에 하나의 메모리를 공유하여 공유 메모리에 있는 운영체제가 모든 처리기들과 시스템 하드웨어를 제어 하는 구조이다. 따라서 공유메모리에 대한 접근 경쟁이 빈번하며, 시간적으로 빠르다. 분산 처리시스템으로 분류한다.
