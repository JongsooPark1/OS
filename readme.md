...
**Swap** : 프로세스가 메모리 공간을 뺏기고 디스크로 나가는 것(Swap out, 보류 상태가 됨)과, 나중에 다시 메모리로 들어오는 것(Swap in)을 통칭해 Swap이라 함

**스레드(Thread)** : 프로세스의 실행 단위. 프로세스 내에 스레드를 만들면서 공유할 수 있는 자원은 공유하면서 각각의 스레드 단위로 관리하는 것이 시간과 비용 측면에서 저렴하다. 한 프로세스 내의 스레드 간 통신은 메모리와 파일을 고유하기 때무에 커널의 개입이 필요없다.

<u>**단일 처리기에서는 스레드 단위로 처리하는 경우와 하나의 프로세스로 처리하는 경우의 차이가 없을까?**</u> => 그렇지 않다. 후자의 경우 순서대로 작업을 처리할 때, 하나의 작업을 처리하고 대기 상태를 거친 후 다른 작업을 진행한다. 이 때 대기 시간이 종료 시간을 늦춘다. 하지만 프로세스 내의 스레드 단위로 작업을 실행하면 하나의 스레드가 대기가 될 때  바로 다른 스레드를 실행시켜 두 번째 호출을 하므로 두 개의 스레드가 대기 중인 시간대가 중복되어 전체 일의 종료 시점을 앞 당길 수 있다.
...
다중처리 : 한 계산의 서로 다른 부분들을 실질적으로 동시에 수행하기 위해 다수의 처리기를 사용하느 ㄴ것을 말하는데, 좁은 의미로는 병렬 처리를, 넓은 의미로는 분산처리를 포함하는 것이다

약결합 : 여러 처리기와 저장 장치(메모리)가 각각 존재해 메모리와 클록을 공유하지 않는 처리기들의 집합이다. 원격프로시저호출(Remote Procedure Call, RPC)로 통신을 하여 서로 다른 시스템의 파일을 참조한다. 처리기 각자는 자신의 메모리에 대한 접근시간은 매우 빠르나 다른 처리기의 메모리는 통신선을 통해 접근해야 하므로 늦다. 일반적으로 분산 처리 시스템으로 분류한다.

분산 시스템의 장점 : 자원의 공유, 연산 속도 향상, 시스템 확장성 증가(필요한 만큼의 소형 시스템들을 추가로 연결), 가격 대비 성능이 우수

분산 시스템의 단점 : 교착 상태 발생 가능, 프로세스 동기화와 통신기법 반드시 제공해야 한다

밀결합 : 여러 처리기 간에 하나의 메모리를 공유하여 공유 메모리에 있는 운영체제가 모든 처리기들과 시스템 하드웨어를 제어 하는 구조이다. 따라서 공유메모리에 대한 접근 경쟁이 빈번하며, 시간적으로 빠르다. 병렬 처리시스템으로 분류한다.

클러스터 : 여러 컴퓨터가 고속의 링크로 연결되어 있으며, 논리적으로는 하나의 컴퓨터인 것처럼 보여지도록 자원을 일관되고 통일되게 관리하는 시스템이다. 구조적으로는 분산 시스템에 가까우나 적은 수의 컴퓨터들을 고속의 네트워크로 묶고, 필요할 경우 공유 디스크를 사용해 접근 성능을 확실히 더 높일 수 있다. 확장성, 가용성이 높으며 결함 허용과 부하 균등화 그리고 병렬계산 등의 기능이 필요하다.

클라우드 컴퓨팅 : 정보처리를 자신의 컴퓨터가 아닌 인터넷으로 연결된 다른 컴퓨터로 처리하는 기술을 말한다.
...
while(true) {
  ...
  while(flag[j] && turn == j); // 다른 프로세스가 진입을 시도하면 대기 아니면 진입
  /* 이곳은 임계영역이다. */
