...
약결합 : 여러 처리기와 저장 장치(메모리)가 각각 존재해 원격프로시저호출(Remote Procedure Call, RPC)로 통신을 하여 서로 다른 시스템의 파일을 참조한다. 처리기 각자는 자신의 메모리에 대한 접근시간은 매우 빠르나 다른 처리기의 메모리는 통신선을 통해 접근해야 하므로 늦다. 일반적으로 분산 처리 시스템으로 분류한다.
